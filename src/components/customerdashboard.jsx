...
const CustomerDashboard = () => {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://api.chefsy.com/profile', {
        method: 'POST',
        body: formData,
      });
  ...
  const cancelBooking = (id) => {
    ...
    setNotifications((prev) => [
      ...prev,
      { id: Date.now(), message: `Booking #${id} cancelled`, date: new Date().toISOString().split('T')[0] },
    ]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gray-50 rounded-2xl shadow-md p-6">
              ...
              <NavLink to="/booking">
                <button className="mb-4 bg-orange-500 text-white font-semibold py-2 px-4 rounded-full hover:bg-orange-600 transition-colors duration-300">
                  Make a New Booking
                </button>
              </NavLink>
              {bookings.length === 0 ? (
